...
@section('content')
    <div class="mb-4 flex justify-between items-center">
        ...
        <a href="{{ route('peminjaman-aset.index') }}" class="px-3 py-2 rounded-lg border text-sm hover:bg-gray-50">
            Kembali
        </a>
    </div>
    ...
    <form method="POST" action="{{ route('peminjaman-aset.update', $data->id) }}"
        ...
        <div>
            ...
            <select name="aset_id" class="mt-1 w-full border rounded-lg border-gray-300 text-sm px-3 py-2" required>
        ...
        <div>
            <div>
                ...
                <select name="kondisi_keluar" class="w-full border rounded-lg border-gray-300 text-sm px-3 py-2 text-sm">
        ...
        <div>
            ...
            <textarea name="tujuan" rows="3" class="w-full border rounded-lg border-gray-300 text-sm px-3 py-2 text-sm">{{ old('tujuan', $data->tujuan) }}</textarea>
            @error('tujuan')
                <div class="text-xs text-red-600 mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        {{-- <div>
            ...
            <textarea name="catatan" rows="4" class="mt-1 w-full border rounded-lg px-3 py-2"
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 pt-2">
            ...
            @if (old('status', $data->status) === 'draft')
                <form method="POST" action="{{ route('peminjaman-aset.destroy', $data->id) }}"
                    ...
                    @csrf
                    @method('DELETE')
                    <button class="px-3 py-2 rounded-lg border border-red-300 bg-red-50 text-red-700 text-sm hover:bg-red-100">
                        Hapus Draft
                    </button>
                </form>
            @endif
