    ...
    <div class="bg-white border rounded p-4 max-w-2xl">
        ...
        <form method="post" action="{{ route('peran.update', $peran) }}" class="space-y-3">
            ...
            <div>
                ...
                <input name="kode" value="{{ $peran->kode }}" class="w-full border rounded px-3 py-2" required>
            </div>

            <div>
                <label class="block text-sm mb-1">Nama</label>
                <input name="nama" value="{{ $peran->nama }}" class="w-full border rounded px-3 py-2" required>
            </div>

            <div>
                <label class="block text-sm mb-1">Deskripsi</label>
                <textarea name="deskripsi" class="w-full border rounded px-3 py-2" rows="2">{{ $peran->deskripsi }}</textarea>
            </div>
