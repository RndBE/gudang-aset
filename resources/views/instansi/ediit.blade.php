    ...
    <div class="bg-white border rounded p-4 max-w-2xl">
        ...
        <form method="post" action="{{ route('instansi.update', $instansi) }}" class="space-y-3">
            ...
            <div>
                ...
                <input name="kode" value="{{ $instansi->kode }}" class="w-full border rounded px-3 py-2" required>
            </div>

            <div>
                <label class="block text-sm mb-1">Nama</label>
                <input name="nama" value="{{ $instansi->nama }}" class="w-full border rounded px-3 py-2" required>
            </div>
