...
@section('content')
    ...
    <form method="post" action="{{ route('lokasi-gudang.store') }}"
        class="bg-white border rounded-lg border-gray-300 p-4 space-y-4">
        @csrf

        <div>
            <label class="text-sm font-medium">Gudang</label>
            <select name="gudang_id" class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" required>
        ...
        <div>
            ...
            <select name="induk_id" class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2">
                <option value="">-</option>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select name="tipe_lokasi" class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" required>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="nama" value="{{ old('nama') }}"
                    class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" maxlength="200">
            </div>
        </div>

        <div>
            <label class="text-sm font-medium">Jalur (opsional)</label>
            <input name="jalur" value="{{ old('jalur') }}"
                class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" maxlength="600"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <select name="bisa_picking" class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2"
                    required>
        ...
        <div class="flex gap-2">
            <a href="{{ route('lokasi-gudang.index', ['gudang_id' => $gudangId]) }}"
                class="px-4 py-2 rounded-lg text-sm border-gray-300 border hover:bg-gray-50 btn-outline-active">Batal</a>
            <button
                class="px-4 py-2 rounded-lg text-sm border-gray-300 bg-gray-900 text-white btn-active cursor-pointer">Simpan</button>
        </div>
    </form>
@endsection
