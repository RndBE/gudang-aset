...
@section('content')
    <div class="bg-white border border-gray-300 rounded-lg p-4 max-w-3xl">
        ...
        <form method="post" action="{{ route('unit-organisasi.update', $unit_organisasi) }}"
            ...
            <div>
                ...
                <x-form-select name="instansi_id" required>
                    @foreach ($instansi as $i)
                        <option value="{{ $i->id }}" @selected($unit_organisasi->instansi_id == $i->id)>
                            {{ $i->nama }}
                        </option>
                    @endforeach
                </x-form-select>
            ...
            <div>
                ...
                <select name="induk_id" class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2">
                    <option value="">-</option>
                    @foreach ($induk as $u)
            ...
            <div>
                ...
                <select name="tipe_unit" class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2" required>
            ...
            <div>
                ...
                <select name="status" class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2" required>
            ...
            <div>
                ...
                <input name="kode" value="{{ $unit_organisasi->kode }}"
                    class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2" required>
            </div>

            <div>
                <label class="block text-sm mb-1">Nama</label>
                <input name="nama" value="{{ $unit_organisasi->nama }}"
                    class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2" required>
            </div>

            <div class="md:col-span-2">
                <label class="block text-sm mb-1">Alamat</label>
                <textarea name="alamat" class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2" rows="2">{{ $unit_organisasi->alamat }}</textarea>
            </div>

            <div>
                <label class="block text-sm mb-1">Telepon</label>
                <input name="telepon" value="{{ $unit_organisasi->telepon }}"
                    class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2">
            </div>

            <div>
                <label class="block text-sm mb-1">Email</label>
                <input name="email" value="{{ $unit_organisasi->email }}"
                    class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2">
            </div>
