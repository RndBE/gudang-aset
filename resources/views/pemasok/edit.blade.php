    ...
    <form method="post" action="{{ route('pemasok.update', $pemasok->id) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="kode" value="{{ old('kode', $pemasok->kode) }}"
                    class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" maxlength="80" required>
            </div>
            <div>
        ...
        <div>
            ...
            <input name="nama" value="{{ old('nama', $pemasok->nama) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="npwp" value="{{ old('npwp', $pemasok->npwp) }}"
                    class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" maxlength="50">
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input name="email" value="{{ old('email', $pemasok->email) }}"
                    class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" maxlength="255">
            </div>
        </div>

        <div>
            <label class="text-sm font-medium">Alamat (opsional)</label>
            <textarea name="alamat" class="mt-1 w-full border rounded-lg text-sm border-gray-300 px-3 py-2" rows="3">{{ old('alamat', $pemasok->alamat) }}</textarea>
        </div>

        <div class="flex gap-2">
            <a href="{{ route('pemasok.index') }}"
                class="px-3 py-2 rounded-lg  border btn-outline-active text-sm hover:bg-gray-50">Kembali</a>
            <button class="px-3 py-2 rounded-lg cursor-pointer text-white text-sm btn-active">Update</button>
        </div>
    </form>
@endsection
