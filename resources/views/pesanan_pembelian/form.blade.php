...
@section('content')
    ...
    <div class="flex items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-semibold">{{ $isEdit ? 'Ubah PO' : 'Buat PO' }}</h1>
            <p class="text-sm text-gray-600 mt-1">Isi header PO dan detail barang. Total akan dihitung otomatis.</p>
        </div>
    ...
    <form id="poForm" method="post"
        ...
        <div class="bg-white border rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
                <div class="font-semibold text-gray-900">Header PO</div>
                @if ($isEdit)
                    @php
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div class="md:col-span-5">
                    ...
                    <select name="pemasok_id" class="w-full border rounded-lg px-3 py-2" {{ $canManage ? '' : 'disabled' }}>
        ...
        <div class="bg-white border rounded-xl p-5 mt-5">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm" id="poTable">
                    ...
                    <tbody id="poTbody">
                        @foreach ($rows as $i => $d)
                            <tr class="border-b po-row">
                                <td class="px-3 py-2">
                                    <select name="barang_id[]" class="w-full border rounded-lg px-2 py-2 barang-id"
                                        {{ $canManage ? '' : 'disabled' }}>
                                        @foreach ($barang as $b)
                        ...
                        <tr>
                            <td colspan="7" class="pt-4">
                                <div class="flex justify-end">
                                    <div class="w-full max-w-md bg-gray-50 border rounded-xl p-4">
                                        ...
                                        <div class="text-xs text-gray-500 mt-2">Perhitungan ini untuk tampilan. Server
                                            tetap menghitung ulang saat simpan.</div>
                                    </div>
                                </div>
                            </td>
                        </tr>
