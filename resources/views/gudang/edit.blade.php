...
@section('content')
    ...
    <form method="post" action="{{ route('gudang.update', $gudang->id) }}" class="bg-white border rounded p-4 space-y-4">
        @csrf
        @method('put')

        <div>
            <label class="text-sm font-medium">Unit Organisasi (opsional)</label>
            <select name="unit_organisasi_id" class="mt-1 w-full border rounded px-3 py-2">
                <option value="">-</option>
                @foreach ($unit as $u)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input name="kode" value="{{ old('kode', $gudang->kode) }}" class="mt-1 w-full border rounded px-3 py-2"
        ...
        <div>
            ...
            <input name="nama" value="{{ old('nama', $gudang->nama) }}" class="mt-1 w-full border rounded px-3 py-2"
                maxlength="200" required>
        </div>

        <div>
            <label class="text-sm font-medium">Alamat (opsional)</label>
            <textarea name="alamat" class="mt-1 w-full border rounded px-3 py-2" rows="3">{{ old('alamat', $gudang->alamat) }}</textarea>
        </div>

        <div class="flex gap-2">
            <a href="{{ route('gudang.index') }}" class="px-3 py-2 rounded border text-sm hover:bg-gray-50">Kembali</a>
